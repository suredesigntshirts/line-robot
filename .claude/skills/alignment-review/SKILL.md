...
2. Spawn ONE fresh `Explore` agent (read-only) with: the surface/diff, the selected context groups, and instructions to read `docs/research/00-product-principles.md` §4 in full — the register's own context headings define which IDs belong to each selected group, and its canonical-merges section defines which ID to cite for duplicated rules.
