---
Source: https://www.pulumi.com/blog/stop-tuning-prompts-build-a-harness/
...
![Stop Tuning Prompts. Build a Harness.](/blog/stop-tuning-prompts-build-a-harness/feature_hu_a486b46ae3748cd1.1b55c7797a6d8f3ed5be8a60baa5aba5aabcb372344044bd1bd169c8c795606f.webp)

# Stop Tuning Prompts. Build a Harness.

 [![Engin Diri](/images/team/engin-diri.jpg)](/community/team/engin-diri/)[Engin Diri](/community/team/engin-diri/)

Posted on May 26th, 2026

Anthropic shipped a piece earlier this month called [How Claude Code Works in Large Codebases](https://claude.com/blog/how-claude-code-works-in-large-codebases-best-practices-and-where-to-start). I have not read anything more useful about coding agents this year. The core claim, in their words: *“the ecosystem built around the model—the harness—determines how Claude Code performs more than the model alone.”* In my phrasing: in a real codebase, the model is the smaller variable. The layer of context and tooling you wire around the agent matters more than which version of Sonnet or Opus is behind it.
...
## How Claude Code navigates without an index[](#how-claude-code-navigates-without-an-index)

Anthropic’s writeup says Claude Code works from the live codebase and does not require a [codebase index](https://claude.com/blog/how-claude-code-works-in-large-codebases-best-practices-and-where-to-start) to be built, maintained, or uploaded. The agent navigates the way an engineer would, with `grep`, `find`, `ls`, file reads, and reference-following. Anthropic calls this [agentic search](https://www.anthropic.com/claude-code), and the upside is obvious: no separate index exists for you to keep fresh.
...
## The AI layer in seven pieces[](#the-ai-layer-in-seven-pieces)
...
## Lean and layered CLAUDE.md[](#lean-and-layered-claudemd)
...
## Hooks that make the harness self-improving[](#hooks-that-make-the-harness-self-improving)
...
## Path-scoped skills[](#path-scoped-skills)
...
## Symbol-level search through LSP and MCP[](#symbol-level-search-through-lsp-and-mcp)
...
![The Claude Code harness as a Gantt-style timeline across a session. CLAUDE.md spans the full session as a foundation. Hooks fire at the start and end. Skills, LSP, and MCP servers light up sporadically. Subagents run as a single longer task between exploration and editing.](./harness-frequency.png)

*Image: [Anthropic, How Claude Code Works in Large Codebases](https://claude.com/blog/how-claude-code-works-in-large-codebases-best-practices-and-where-to-start).*

## Subagents for exploration[](#subagents-for-exploration)
...
## Don’t let it rot[](#dont-let-it-rot)
...
## Assign an owner[](#assign-an-owner)
