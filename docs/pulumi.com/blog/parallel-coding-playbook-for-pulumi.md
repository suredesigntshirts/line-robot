---
Source: https://www.pulumi.com/blog/parallel-coding-playbook-for-pulumi/
...
![Five Stacks Before Lunch: The Parallel Coding Playbook for Pulumi](/blog/parallel-coding-playbook-for-pulumi/feature_hu_2f3bd1295d12bed8.82dd84d10d3250be73800a8e53b6c88c54751fe025c8581e5ef6fbe725c7ac93.webp)
...
## 2x is prompting, 10x is plumbing[](#2x-is-prompting-10x-is-plumbing)
...
The application-code version of this playbook leans on ports, node\_modules, and databases to get isolation right. The infrastructure version has a different toolbox.

## What changes when the agents are touching infrastructure[](#what-changes-when-the-agents-are-touching-infrastructure)
...
## The five catches, infra edition[](#the-five-catches-infra-edition)
...
Every parallelism story has a catch list. The application-code version lists port conflicts, node\_modules sprawl, database conflicts, token blowouts, and PR pile-up. The infrastructure equivalents map almost one to one.
...
**node\_modules sprawl becomes provider plugin sprawl, mostly already solved.** Three worktrees each pulling their own copy of `pulumi-aws` would add up fast, except [Pulumi already shares plugins through a single cache](/docs/iac/concepts/plugins/) at `~/.pulumi/plugins`. Identical provider versions are reused across worktrees automatically. Per-worktree language SDKs (`node_modules`, `venv`) still need the usual care, but the provider layer is free.
...
## Where to start, this afternoon[](#where-to-start-this-afternoon)
...
## Five stacks before lunch[](#five-stacks-before-lunch)
...
[See how Pulumi Neo runs your stacks](/product/neo/)

-   [ai](/blog/tag/ai/)
-   [ai-agents](/blog/tag/ai-agents/)
-   [automation](/blog/tag/automation/)
