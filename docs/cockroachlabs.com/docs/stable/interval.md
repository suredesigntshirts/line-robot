---
Source: https://www.cockroachlabs.com/docs/stable/interval
...
-   [Syntax](#syntax)
    -   [Details on SQL Standard input](#details-on-sql-standard-input)
-   [Size](#size)
-   [Precision](#precision)
...
The `INTERVAL` [data type](/docs/v26.2/data-types) stores a value that represents a span of time.

## Syntax

You can express a constant value of type `INTERVAL` using an [interpreted literal](/docs/v26.2/sql-constants#interpreted-literals), or a string literal [annotated with](/docs/v26.2/scalar-expressions#explicitly-typed-expressions) type `INTERVAL` or [coerced to](/docs/v26.2/scalar-expressions#explicit-type-coercions) type `INTERVAL`. CockroachDB also supports using uninterpreted [string literals](/docs/v26.2/sql-constants#string-literals) in contexts where an `INTERVAL` value is otherwise expected.

`INTERVAL` constants can be expressed using the following formats:

| Format | Description |
| --- | --- |
| SQL Standard | `INTERVAL 'Y-M D H:M:S'` , You can express seconds and days as integers or floats. All other input values must be expressed as integers. , For more details, see [Details on SQL Standard input](#details-on-sql-standard-input). |
...
By default, CockroachDB displays `INTERVAL` values in the traditional PostgreSQL format (e.g., `INTERVAL '1 year 2 months 3 days 4 hours 5 minutes 6 seconds'`). To change the display format of `INTERVAL` values, set the `intervalstyle` [session variable](/docs/v26.2/set-vars) or the `sql.defaults.intervalstyle` [cluster setting](/docs/v26.2/cluster-settings) to a supported format (`iso_8601` for the ISO 8601 format; `sql_standard` for the SQL Standard format).
...
You can [cast](/docs/v26.2/data-types#data-type-conversions-and-casts) `INTERVAL` values to any of the following data types:
