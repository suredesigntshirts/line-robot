---
Source: https://www.cockroachlabs.com/docs/stable/time
...
-   [Precision](#precision)
-   [Examples](#examples)
    -   [Create a table with a `TIME`\-typed column](#create-a-table-with-a-time-typed-column)
    -   [Create a table with a `TIME`\-typed column, with precision](#create-a-table-with-a-time-typed-column-with-precision)
-   [Supported casting & conversion](#supported-casting-conversion)
-   [See also](#see-also)

The `TIME` [data type](/docs/v26.2/data-types) stores the time of day in UTC.
...
-   `TIMETZ`, which converts `TIME` values with a specified time zone offset from UTC.
    ...
    [Like PostgreSQL](https://www.postgresql.org/docs/current/static/datatype-datetime.html), we implement the `TIMETZ` variant for [SQL standards compliance](/docs/v26.2/sql-feature-support). We also implement the `TIMETZ` variant for compatibility with ORMs, like [Hibernate](/docs/v26.2/build-a-java-app-with-cockroachdb-hibernate).

You can use the [`timezone()`](/docs/v26.2/functions-and-operators#date-and-time-functions) and [`AT TIME ZONE`](/docs/v26.2/functions-and-operators#special-syntax-forms) functions to convert a `TIMETZ` into a `TIME` at a specified timezone, or to convert a `TIME` into a `TIMETZ` at a specified timezone.
...
A constant value of type `TIME` can be expressed using an [interpreted literal](/docs/v26.2/sql-constants#interpreted-literals), or a string literal [annotated with](/docs/v26.2/scalar-expressions#explicitly-typed-expressions) type `TIME` or [coerced to](/docs/v26.2/scalar-expressions#explicit-type-coercions) type `TIME`. When it is unambiguous, a simple unannotated [string literal](/docs/v26.2/sql-constants#string-literals) can also be automatically interpreted as type `TIME`.
...
A time zone offset of `+00:00` is also displayed for all `TIME` and [`TIMESTAMP`](/docs/v26.2/timestamp) values, but is not stored in the database.
...
If no time zone is specified for a `TIMETZ` value, the `timezone` [session variable](/docs/v26.2/show-vars#supported-variables) is used. For example, if you [set the `timezone`](/docs/v26.2/set-vars#set-time-zone) for a session using `SET TIME ZONE -2`, and you define the `TIMETZ` as `TIMETZ '10:10:10.55'`, the value will be displayed with an offset of -2 from UTC.

`TIMETZ` is not affected by session-scoped offsets (unlike [`TIMESTAMPTZ`](/docs/v26.2/timestamp)). Time zone offsets only apply to values inserted after the offset has been set, and do not affect existing `TIMETZ` values, or `TIMETZ` values with a time zone offset specified.
...
You can use an [`ALTER COLUMN ... SET DATA TYPE`](/docs/v26.2/alter-table#alter-column) statement to change the precision level of a `TIME`\-typed column. If there is already a non-default precision level specified for the column, the precision level can only be changed to an equal or greater precision level. For an example, see [Create a table with a `TIME`\-typed column, with precision](#create-a-table-with-a-time-typed-column-with-precision).
...
`TIME`/`TIMETZ` values can be [cast](/docs/v26.2/data-types#data-type-conversions-and-casts) to any of the following data types:
...
CockroachDB displays `TIME '24:00:00'` and `TIMETZ '24:00:00'` as `0000-01-01 00:00:00`. To display the proper stored value (`24:00:00`), you can [cast the data type to a `STRING`](/docs/v26.2/time#supported-casting-conversion).

## See also

-   [Data Types](/docs/v26.2/data-types)
-   [SQL Feature Support](/docs/v26.2/sql-feature-support)
