...
|---|---|---|
| 2026-06-13 | Build amendments (Increments 4+5): (a) D-S1-7 clarification — "multi-property dumps" are the caller's spec-list (`generateCase(specs[])`), not a ChaosProfile toggle; the dump IS the input shape (spec auditor judged faithful). (b) CKAN spider: data.go.th unreachable from the build sandbox → adapter is fixture-tested with injectable fetch; live pull = set `CKAN_DATASTORE_URL` (morning option). (c) Panel fixes: mid-thread-correction rendered from a stale-price spec instead of RNG-fragile string replacement; listing.source_group_id = owner's actual group; `num("")` no longer invents 0; db:seed try/finally + exit code. | Increment review |
