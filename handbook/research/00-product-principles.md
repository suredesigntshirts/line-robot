...
- *Addendum 2026-09-05:* `/alignment-review` is archived (`handbook/archive/claude/`); reviewers read §4 directly (root `CLAUDE.md` rule 9), and `/frontend-review` covers the visual side.
- *Addendum 2026-09-05:* paths cited in frozen bodies predate the repo reorganisation — `spikes/*/FINDINGS.md` → `handbook/archive/spikes/`, `plans/NN-*` → `handbook/archive/plans/`, `docs/design|research` → `handbook/design|research`.
- Known weakest evidence (carried from artifact Confidence sections): in-group etiquette (A1), nomad counts/yields (A2), discount-to-close timing (A3), CNX-specific CAM fees (A4), ~~DDproperty CTA order (A5, Cloudflare-blocked)~~ — RESOLVED 2026-06-12 by first-hand capture (see A5 addendum: DDProperty mobile is enquiry-funnel-first, not LINE-first; our LINE-first CTA stands as differentiation), auth-abandonment ranking (B1), map-vs-list split (B2), particle rule single-source (B3), astro-sst+Astro-6 compat untested (C1). Treat these as defaults-to-test, not facts.
