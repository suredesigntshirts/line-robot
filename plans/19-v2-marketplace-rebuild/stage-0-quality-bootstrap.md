...
| 2026-06-13 | I5 build amendment: Increment 5's component list omitted two master-§7 kept items (Pulumi setup + deploy identity; test harness patterns) — spec-auditor [MAJOR] vs the "every §7 kept component" acceptance criterion. Resolved by ADDING both audits (rows 9–10, both KEEP) rather than narrowing the criterion; the audit covers the full §7 set (10 verdicts). | Increment review finding |
| 2026-06-13 | I4 build amendments: (a) runner entry is `node src/eval/runner.ts` (Node 24 native type-stripping) instead of `tsx` — zero new dependency; forces `.ts`-extension imports + `allowImportingTsExtensions` inside packages/pipeline only. (b) Deterministic scorers (exact, numeric-tolerance, dedup pairs, property-count) ship as real frozen-semantics implementations WITH unit tests (12) rather than placeholder stubs — panel-reviewed (spec auditor accepted; simplicity critic demanded the tests; skeptic confirmed keep+test over gut-to-stubs, which Stage 2 would just rewrite). scoreFuzzy (LLM judge) and photo-attribution scoring remain Stage 2. | Increment review findings |
