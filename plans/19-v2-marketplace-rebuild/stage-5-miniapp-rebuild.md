...
- My listings screen: user's claimed listings with lifecycle status; mini-app edit entry point (NOT edit-by-reply — retired per A3a)
...
- Playwright smoke covers the claim flow end-to-end (requires a test LIFF context or mock — define the test strategy in the fleshed spec); also covers my listings, saved listings, and the mini-app edit surface
...
| 2026-06-14 | **Build D DONE (per-user CRM, D13).** Built the saved tab (`GET /me/saved` → `MyListingCard`s + empty state), the viewings tab (`GET /me/viewings` → upcoming/past timeline sections authored to `explore-stage5-3-viewings.html`, new `ViewingCard`), the detail-screen CRM widgets (save/unsave optimistic toggle with rollback; `BookViewing` native `datetime-local` → `POST /properties/{id}/viewings` with future-time + 400 handling; `NotesSection` list + add + empty-note 400/client validation), and the owner edit surface (`/edit/{id}` additive route → `EditScreen` minimal-patch form → `PATCH /properties/{id}`; sale shows priceThb, rent shows monthlyRent; 404 non-claimant + 400/500 paths). Saved/viewings are TABS within `/` (not routes — deep links never target them); `/edit/{id}` is additive; the two frozen shapes are untouched. Removed the now-orphaned `ComingSoon` + its catalog keys. Added 41 unit + 16 e2e (×light/dark) tests; full cadence GREEN (typecheck/lint/93 unit/40 e2e). DF-6 stays descoped (queued). | Stage-5 build order (A→E); Build D taken to completion with its own tests before review |
| 2026-06-14 | **Build D panel-fix pass (7 review findings applied).** (1) **isSaved end-to-end** — `getPortalListingDetail(id, callerUserId)` now computes a per-caller `isSaved` (correlated EXISTS over `saved_listing`; literal `"listing".id` correlation — the projection-subquery gotcha bit once in test); surfaced in the api detail DTO + the miniapp `ListingDetailDto`; `SaveToggle` seeds from it (a saved listing renders "saved" on revisit; first tap un-saves). (2) `apiStatus(err)` helper replaces the copy-pasted `instanceof ApiError` guard at all 6 ErrorView sites. (3) Hoisted ONE shared `<Outcome>` (success/warn/danger) used by Claim + Edit (deleted both local copies; uses `primaryButtonClass`). (4) LEGAL-06 "poster-provided, verify" line added to `MyListingCard` (register §4 "on cards"). (5) BookViewing comment fixed + server-400 → `viewing.errorInvalid`; cancel resets the value. (6) Edit negative-number guard (`min` on inputs + `nonNegInt` in `buildPatch`). (7) cleanups: `addressDetail`-omission comment, `intlLocale()` helper, `MessageKey`-typed `viewingStatusKey`. Full gate GREEN: typecheck (all pkgs) / lint / 94 miniapp + 46 api unit / 42 db integration (Docker) / 42 e2e. **Founder taste calls still queued (not self-adjudicated):** Buddhist-era dates (native th-TH Intl); the edit field set (= api allowlist, addressDetail deliberately omitted); the owner-own-card LEGAL-06 redundancy; TH-03 trust signal / "นัดชม" term / website-card LEGAL-06 parity. | Stage-5 review panel: spec PASS, correctness/simplicity findings to apply before the increment lands |
