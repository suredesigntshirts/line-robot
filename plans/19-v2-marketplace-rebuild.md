...
| 4b | `v2-catalog-cutover.md` | **Full v2 launch** (founder ruling 2026-06-13: fully migrate, retire v1). Flip `PIPELINE_V2=on` + move ALL v1 catalog readers to Postgres via one shared `PostgresPropertyStore` + `listing_event` table: bot processor, reminder, AND the **mini-app read-api** (Path A — Preact SPA unchanged, no Stage 5 rebuild needed for the data migration). Includes the small, proven image/classify/OCR increment. v1 catalog abandoned (no backfill, D2). Stage 5 = the optional React UI rebuild, decoupled | Real data live on website + mini-app + in-chat; v1 retired |
