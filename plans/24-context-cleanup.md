...
9. **Boring, predictable names.** `STATUS.md`, `DECISIONS.md`, `docs/runbooks/`, `docs/archive/`.
...
docs/
  ...
  product/
    ...
    heuristics.md              → keep docs/research/00-product-principles.md as-is, just link it
  design/                      keep (mockups, renders, tokens, moodboard). README already indexes.
  research/                    keep. Freeze every header to "Frozen <date>; register live".
  archive/
    2026-06-sprint/            SPRINT-LOG.md, MORNING.md, BLOCKERS.md, BACKLOG.md (moved verbatim)
    plans/                     plans 00–23 + plans/cleanup/ + skill-hardening logs, moved verbatim,
                               plus README.md: a table "plan · what it was · outcome · superseded by"
    spikes/                    the two FINDINGS.md only (drop the checked-in spike app + its
                               269 MB untracked node_modules)
  <vendor doc caches>          keep only what the stack uses: astro, tailwind, shadcn, drizzle,
                               postgres, postgis, node-postgres, pulumi, aws, line, anthropic.
                               Delete bun/cockroach/turso/neon/netlify/cloudflare/effect/expo/
                               sqlite/nile/microsoft caches; regenerate docs/llms.txt.
plans/
  README.md                    "One active plan at a time. Archived plans → docs/archive/plans."
...
Root files to move or delete: `KHAIFAK.md` → `docs/research/khai-fak-sources.md`;
`WEBSITES.md` → fold into `docs/research/a5-competitor-teardown.md` sources or delete;
`line-mini-app-qr*.png` → `docs/archive/`; `LINE.md` (gitignored plaintext credentials) → the
...
3. **Archive history** by `git mv` (never edit archived bodies): the 4 status files, plans 00–23,
   `plans/cleanup/`, `docs/design/skill-hardening/`, spikes. Write `docs/archive/plans/README.md`
...
5. **Freeze research headers** (`docs/research/*`, `00-product-principles.md`): one header line
   each, no body edits. Fix `docs/design/design-direction.md` is already reconciled — verify.
...
8. **Vendor doc caches**: delete the unused folders; regenerate `docs/llms.txt`.
...
| 3 | Archive vs delete history | **Archive plans/logs under `docs/archive/` with an outcome index; delete the memory files and vendor caches** (git history keeps everything). |
