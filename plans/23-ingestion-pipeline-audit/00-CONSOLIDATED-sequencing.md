...
> Status: RECONCILED · Inputs: the four group R+P artifacts (A/B/C/D) in this folder + `plans/23-ingestion-pipeline-audit.md` · Phase: still Plan (no code executed here)
...
```
            ...
            └───────────────────┬─────────────────────────┘
                                ...
                                ▼
            ┌─────────────────────────────────────────────┐
            │ Phase 3: Group B RESILIENCE  (+E10, +E8-hook)│  (retract the band-aid)
            │  classify-cache → chunk+watermark → failure  │  ← rebases on A's sweep edit
            │  signal → lower Lambda 900→~300s → idempotency│
            └───────────────────┬─────────────────────────┘
...
| **CR-5** | Per-step tracing/timing E8 (B↔C) — *duplicate-build risk* | B wants "which step ate the timeout budget"; C wants hill-climb diagnostics. Both would add per-step timing. | **Build ONE thin hook** (extend `cost.ts` `CostEntry` with `latencyMs` + an optional `traceSink`) in **P3**; C consumes it as eval/scorecard outputs. The full per-conversation trace *view* is deferred (P5). | Pipeline infra (built w/ B, consumed by C) · P3 / P5 |
...
| **E8** per-step tracing/timing | B + C both wanted it | One thin hook (P3) · full trace view (P5) | **P3 hook / P5 view** |
...
### Phase 3 — Performance & resilience (Group B) + E10 + the E8 hook · *retract the band-aid*
**Why third:** the band-aid holds, so this is not silent-loss-urgent; it rebases on A's settled sweep
(CR-1) and benefits from A's safe-under-any-batching dedup (CR-2). **Spec:** group-b §5 (Steps B, A, C).
- **B (classify cache, do first):** persist the classify verdict keyed by `s3Key` — **B1: a DynamoDB
  attribute on the message item** (CR / §6) — read-through in `run.ts:182-185`. Warm re-runs do **zero**
  classify calls.
- **A (chunk + watermark-per-chunk):** cap one `ingestOne` to the first C images (default ≈12,
  env-overridable); advance the watermark per chunk; reset attempts on a successful chunk.
...
- **Then lower `infra/src/lambdas.ts` 900→~300s** and fix the `naming.ts:31` comment (CR-13).
- **E10 (CR-4):** a stable write idempotency key so a re-run reconciles instead of duplicating.
- **E8 hook (CR-5):** `latencyMs` per `CostEntry` + optional `traceSink`, so caching/chunking wins are
  measurable (classify-call-count cold-vs-warm; per-step timing).
- **Gate:** §5.3 (backend, **no** `/frontend-review`); the new **pipeline integration test** (Docker-PG
  ...
  work; post-deploy staging check that a real multi-image dump drains over several ticks + a warm
...
(Option 1).
- New nullable `listing.dm_claimant_user_id` FK (migration `0011`, domain-enum N/A, hand-fix per
...
- **E8 full trace view** + **E6 capture consumer** (CR-5/CR-6) — once the P3 hook + signal exist.
...
2. **The per-step timing/trace hook** (P3) — B uses it for chunking/caching validation; C surfaces it as
   scorecard outputs. (CR-5)
...
5. **Chunk default ≈12 images** + **target Lambda timeout ≈300s** after chunking — tune via the replay
   test, not by hand. *(Founder/ops confirm the numbers.)*
