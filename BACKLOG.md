...
| 2 Extraction pipeline | Code-complete, **flipped live today** (`PIPELINE_V2=on`). **NOT formally gated.** Real tail. |
...
1. **Finish the v2 cutover so the flip actually pays off** — **A1 (repoint readers to Postgres)** is
   the one forward-blocking item: the pipeline now writes Postgres, but the bot's in-chat commands and
   the mini-app still read the frozen v1 DynamoDB catalog, so new listings are invisible there. Path A
   is small (reuse `PostgresPropertyStore`, SPA unchanged) + the `listing_event` table.
...
| # | Item | Priority |
|---|---|---|
| A1 | **Repoint catalog readers to Postgres** — bot processor + reminder + read-api/mini-app. Build `PostgresPropertyStore` (the `PropertyStore` slice; `ConversationStore` stays DynamoDB), split the port, add the **`listing_event`** table (+ partial index — the `viewing` table does NOT cover follow-up reminders). Read-api repoint = one `read-api.ts buildDeps()` line + `DATABASE_URL`; Preact SPA unchanged. _Design salvaged into the Stage 2 cutover section._ | **BLOCKING** (flip delivers nothing to mini-app/in-chat without it) |
