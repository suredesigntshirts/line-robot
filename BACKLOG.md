...
| 2 Extraction pipeline | Live, **v2-only sweep**. **A1 readers, A2 image/OCR, A3 v1-extractor deletion** deployed + verified; **A4 batch transport proven on staging** (50% pricing + cache hit; cron wiring deferred → A4d). **NOT formally gated.** Tail: A5 (cutover hardening), A8 (gate). |
...
2. **Clear the Stage 2 fast-follow cluster:** ~~A2 image+OCR~~, ~~**A3** delete `claudeExtractor` + the v1 extraction stack~~, ~~**A4** batch warm-cache acceptance~~ **(all done; batch cron wiring deferred — A4d)**; remaining — **A5** cutover hardening.
