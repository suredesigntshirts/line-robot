...
| 2 Extraction pipeline | Live, **v2-only sweep** (`PIPELINE_V2` flag retired in A3). **A1 readers, A2 image/OCR, A3 v1-extractor deletion** all deployed + verified. **NOT formally gated.** Tail: A4 (batch routing), A5 (cutover hardening), A8 (gate). |
...
2. **Clear the Stage 2 fast-follow cluster:** ~~A2 image+OCR wiring~~, ~~**A3** delete `claudeExtractor` + the v1 extraction stack~~ **(both done, deployed + verified)**; remaining —
   **A4** batch routing + warm-cache check, **A5** cutover hardening.
