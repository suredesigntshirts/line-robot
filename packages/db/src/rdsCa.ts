// AWS RDS regional CA bundle (ap-southeast-1), fetched from
// https://truststore.pki.rds.amazonaws.com/ap-southeast-1/ap-southeast-1-bundle.pem
// Embedded so the Lambda bundle can verify the RDS server cert (verify-full):
// RDS's CA is not in Node's default trust store. Refresh before the bundle's
// expiry (these certs are valid into the 2060s) or if the instance's CA rotates.
const RDS_CA_BUNDLE = `
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`;

/**
 * pg connection options for a DATABASE_URL. RDS hosts get full TLS verification
 * against the bundled CA; anything else (local Docker Postgres in tests) connects
 * unchanged. This is the one place TLS for Postgres is decided.
 *
 * Any `sslmode` in the URL is stripped for RDS: pg 8.x treats `require` as
 * `verify-full` and an in-URL `sslmode` takes precedence over the explicit `ssl`
 * object, so leaving it in would bypass our CA and fail with SELF_SIGNED_CERT.
 */
export function pgConnectionConfig(connectionString: string): {
  connectionString: string;
  ssl?: { ca: string; rejectUnauthorized: true };
} {
  if (!connectionString.includes("rds.amazonaws.com")) return { connectionString };
  return {
    connectionString: connectionString.replace(/[?&]sslmode=[^&]*/g, ""),
    ssl: { ca: RDS_CA_BUNDLE, rejectUnauthorized: true },
  };
}
